...
public:
    int maxTaskAssign(vector<int>& tasks, vector<int>& workers, int pills, int strength) {
        ...
        while(l < r) {
            ...
            3) only uses pills on those who absolutely need it , and uses it in the most sensible scenario : gives extra strength to him who needs it (weakest guy) , for a task that needs it (biggest task)
            */
            int m = l + ((r-l)>>1);
            int i = m - 1;
            int cnt = pills;
