...
public:
    vector<int> treeQueries(int n, vector<vector<int>>& edges, vector<vector<int>>& queries) {
        ...
        //all indexing is done using node2Ind
        int index = 0;
        auto flatten = [&](int src , int par , int cost , auto &&self) -> int{
            node2Ind[src] = index++;
            int IND_SRC = node2Ind[src];
            data.emplace_back(src , cost);
            int subtreeSz = 0;
            for(auto &[nxt,wt] : adj[src]){
                if(!(nxt^par)) continue;
                subtreeSz += self(nxt , src , cost + wt , self);
                int IND_NXT = node2Ind[nxt];
                node2Par[IND_NXT] = {src , wt};
            }
            node2Sz[IND_SRC] = subtreeSz;
            return max(subtreeSz , 1);
        };
        flatten(0 , -1 , 0 , flatten);
        // for(int i = 0 ; i < n ; ++i){
        //     auto [node , cost] = data[i];
        //     cout << "NODE: " << i << " " << node2Ind[i] << " " << node2Sz[i] << " " << node2Par[i].first << " " << node2Par[i].second << " " << 
        //     // cout << node << " " << cost << " " << node2Ind[i] << " " << ind2Sz[node2Ind[i]] << endl;
        // }
        ...
        for(auto &q : queries){
            if(q[0] == 2){
                ...
                --q[1] , --q[2];
                int IND1 = node2Ind[q[1]];
                int IND2 = node2Ind[q[2]];
                int child = (node2Par[IND1].first == q[2]) ? IND1 : IND2;
                int sz = node2Sz[child];
                int delta = q[3] - node2Par[child].second;
                lz.update(child , child + sz , delta);
