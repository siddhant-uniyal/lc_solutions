...
public:
    vector<int> getOrder(vector<vector<int>>& tasks) {
        ...
        vector<int>ans(n);
        long thresh = tasks[ids[0]][0];
        int j = 1;
        for(int i = 0 ; i < n ; ++i){
            if(pq.empty()) pq.emplace(tasks[ids[i]][1] , ids[i]) , j = i + 1;
            auto [cost , src] = pq.top();
            pq.pop();
            ans[i] = src;
            // thresh = max(tasks[src][0] + cost , thresh);
            thresh += cost;
            while(j < n && tasks[ids[j]][0] <= thresh){
                pq.emplace(tasks[ids[j]][1] , ids[j]);
                // thresh = max(tasks[ids[j]][0] + tasks[ids[j]][1] , thresh);
                ++j;
            }
        }
        return ans;
